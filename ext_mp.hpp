 ...
 * we move to explicit multiparenthesis when sigma <= 15
 */
#include <algorithm>
#include <cassert>
...
private:
	...
	_query( node_type x, node_type y, node_type z, const value_type wc, size_type k ) const {
		...
		if ( b-a+1 <= SIGMA_LIMIT ) 
			return mh->query(x,y,z,k);
	...
	void init( const std::string &s, const std::vector<value_type> &wgt ) {
		...
		if ( b-a+1 <= SIGMA_LIMIT ) {
			mh= new multiple_hierarchies<>(s,wgt);
			return ;
		}
		...
		assert( st.empty() );
		if ( !vec[0].empty() )
			t[0]= new ext_mp<>(str[0].str(),vec[0],a,mid,B[0]);
		else t[0]= NULL;
		if ( !vec[1].empty() )
			t[1]= new ext_mp<>(str[1].str(),vec[1],mid+1,b,B[1]);
		else t[1]= NULL;
	}

	ext_mp( const std::string &s, const std::vector<value_type> &wgt, value_type a, value_type b, const t_bp_support *stru ) {
...
public:
	...
	ext_mp( const std::string &s, const std::vector<value_type> &wgt ) {
		n= s.length()/2;
		value_type mi= 0;
		assert( s.length()/2 == wgt.size() );
		value_type sigma= 0;
	...
	*/
	...
	double bits_per_node() const {
		...
		double ans= 8*(sdsl::size_in_bytes(*structure)+(is_homogeneous()||b-a+1<=SIGMA_LIMIT?0:S->size_in_bytes())+sizeof(a)+sizeof(b));
