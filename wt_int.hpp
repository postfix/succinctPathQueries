...
class wt_int
{
    ...
    private:
		...
		static bool is_empty( const std::pair<size_type,size_type> &p ) {
		...
		_range_quantile( std::vector<std::pair<size_type,size_type>> &r, size_type k,
			...
			if ( level >= m_max_level ) {
				...
				for ( auto i = 0; i < r.size(); ++i ) {
					...
					if ( m_cnt.find(paths[i]) == m_cnt.end() )
						m_cnt[paths[i]] = r[i].second-r[i].first;
					else m_cnt[paths[i]] += r[i].second-r[i].first;
				}
				for ( auto it: m_cnt )
					if ( k < it.second )
						return it.first;
			...
			std::vector<size_type> ones_before_o(r.size()); // = m_tree_rank(offset);
			for ( auto i = 0; i < ones_before_o.size(); ++i ) {
				if ( is_empty(r[i]) ) continue ;
				ones_before_o[i] = m_tree_rank(offsets[i]);
			}

			std::vector<size_type> ones_before_i(r.size()); //m_tree_rank(offset+i) - ones_before_o;
			for ( auto i = 0; i < ones_before_i.size(); ++i ) {
				if ( is_empty(r[i]) ) continue ;
				ones_before_i[i] = m_tree_rank(offsets[i]+r[i].first)-ones_before_o[i];
			}

			std::vector<size_type> ones_before_j(r.size()); //m_tree_rank(offset+j) - ones_before_o;
			for ( auto i = 0; i < ones_before_j.size(); ++i ) {
				if ( is_empty(r[i]) ) continue ;
				ones_before_j[i] = m_tree_rank(offsets[i]+r[i].second)-ones_before_o[i];
			}

			std::vector<size_type> ones_before_end(r.size()); //m_tree_rank(offset+ node_size) - ones_before_o;
			for ( auto i = 0; i < ones_before_end.size(); ++i ) {
				if ( is_empty(r[i]) ) continue ;
				ones_before_end[i] = m_tree_rank(offsets[i]+node_sizes[i])-ones_before_o[i];
			}

			size_type accum_left = 0, accum_right = 0;
			for ( auto i = 0; i < r.size(); ++i ) {
				if ( is_empty(r[i]) ) continue ;
				accum_left  += (r[i].second-r[i].first)-(ones_before_j[i]-ones_before_i[i]);
				accum_right += (ones_before_j[i]-ones_before_i[i]);
			}

			assert( k < accum_left || k-accum_left < accum_right );

			if ( k < accum_left ) {
				for ( auto i = 0; i < r.size(); ++i ) {
					offsets[i] = offsets[i]+m_size;
					node_sizes[i] = node_sizes[i]-ones_before_end[i];
					r[i].first = r[i].first-ones_before_i[i];
					r[i].second = r[i].second-ones_before_j[i];
					paths[i] <<= 1;
				}
				return _range_quantile(r,k,level+1,paths,node_sizes,offsets);
			}
			assert( k-accum_left < accum_right );

			for ( auto i = 0; i < r.size(); ++i ) {
				offsets[i] = offsets[i]+(node_sizes[i]-ones_before_end[i])+m_size;
				node_sizes[i] = ones_before_end[i];
				r[i].first = ones_before_i[i];
				r[i].second = ones_before_j[i];
				paths[i] <<= 1, ++paths[i];
			}
    ...
    public:
		...
		range_quantile( std::vector<std::pair<size_type,size_type>> &r, size_type k ) const {
			std::vector<size_type> paths(r.size(),0),
								   node_sizes(r.size(),m_size),
								   offsets(r.size(),0);
