...
public class RandBracketSequence {
	...
	public static void main( String [] args ) throws Exception {
		if ( args.length == 0 ) 
			...
			new RandBracketSequence().generate(n,m);
		}
	}
	private boolean isCBS( String w ) {
		if ( w == null || w.length() == 0 )
			return true ;
		int i,j,k,s = 0,n = w.length();
		for ( i = 0; i < n; ++i ) 
			if ( (s += w.charAt(i)==OPENING_BRACKET?1:-1) < 0 )
				return false;
		return s == 0;
	}
	...
	void generate( int n, int m ) throws Exception {
		for ( int i = 0; i < m; ++i ) 
			System.out.println(randomBPS(n));
	}
	void go() throws Exception {
		for ( int n = 2; n <= 13; ++n ) {
			Map<String,Integer> cnt = new HashMap<>();
			for ( int i = 0; i < (1<<21); ++i ) {
				String r = randomBPS(n);
				assert isCBS(r);
