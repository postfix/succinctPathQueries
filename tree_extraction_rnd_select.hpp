...
class tree_extraction_rnd_select: public path_query_processor {
private:
	//static const uint32_t K= 0x400;
	static constexpr rnd_select<> rs= rnd_select<>();
	bool is_final_tree;
	...
	tree_extraction_rnd_select *t[r];
	...
	_query( node_type x, node_type y, node_type z, const value_type wc, size_type k ) const {
		...
		if ( is_final_tree ) {
			...
			if ( z < +oo && wc == weights[z] )
				path.push_back(wc);
			return rs.select(path,static_cast<size_type>(0),static_cast<size_type>((path.size()-1)),static_cast<size_type>(path.size()>>1));
		}
	...
	void init( const std::string &s, const std::vector<value_type> &wgt ) {
		...
		assert( st.empty() );
		if ( !vec[0].empty() )
			t[0]= new tree_extraction_rnd_select<>(str[0].str(),vec[0],a,mid);
		else t[0]= NULL;
		if ( !vec[1].empty() )
			t[1]= new tree_extraction_rnd_select<>(str[1].str(),vec[1],mid+1,b);
		else t[1]= NULL;
	}

	tree_extraction_rnd_select( const std::string &s, const std::vector<value_type> &wgt, value_type a, value_type b ) {
...
public:
	...
	tree_extraction_rnd_select( const std::string &s, const std::vector<value_type> &wgt ) {
		n= s.length()/2;
		assert( s.length()/2 == wgt.size() );
		value_type sigma= 0;
