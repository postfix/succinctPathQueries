...
class tree_extraction_wo_dummy: public path_query_processor {
private:
	...
	void init( const std::string &s, const std::vector<value_type> &wgt ) {
		...
		assert( st.empty() );
		if ( !vec[0].empty() )
			t[0]= new tree_extraction_wo_dummy<>(str[0].str(),vec[0],a,mid);
		else t[0]= NULL;
		if ( !vec[1].empty() )
			t[1]= new tree_extraction_wo_dummy<>(str[1].str(),vec[1],mid+1,b);
		else t[1]= NULL;
	}

	tree_extraction_wo_dummy( const std::string &s, const std::vector<value_type> &wgt, value_type a, value_type b ) {
...
public:
	...
	tree_extraction_wo_dummy( const std::string &s, const std::vector<value_type> &wgt ) {
		n= s.length()/2;
		assert( s.length()/2 == wgt.size() );
		value_type sigma= 0;
